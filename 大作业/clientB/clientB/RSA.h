...
string private_key_B_d = "815B924BC5F56394CD9A4F0241232993F";
//拥有对方的公钥
...
//用对方的公钥加密
//在这里传进来的时候就是16进制数
string encrypt_A_public(string s) {
	// 传进16进制数，所以最多16位
	if (s.size() > 16) {
	...
	BigInt b_d = string2BigInt(public_key_A_e);
...
//用A的公钥解密
string decrypt_A_public(string s) {
...
//用B的私钥解密
string decrypt_B_private(string s) {
	
	BigInt b = string2BigInt(s);

	BigInt b_d = string2BigInt(private_key_B_d);
...
string decrypt_B_public(string s) {
	BigInt b = string2BigInt(s);
